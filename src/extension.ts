import { window, ExtensionContext, commands, languages, workspace } from "vscode";
...
import { Client } from "./view/ui/Client";
...
export async function activate(context: ExtensionContext) {
	...
	if (resSdk.isError()) {
		window.showErrorMessage(`Geode: Unable to setup Geode extension: ${resSdk.unwrapErr()}`);
	...
	if (resCLI.isError()) {
		window.showErrorMessage(`Geode: Unable to setup Geode extension: ${resCLI.unwrapErr()}`);
	...
	if (resProjects.isError()) {
		window.showErrorMessage(`Geode: Unable to setup Geode extension: ${resProjects.unwrapErr()}`);
	...
	if (resBrowser.isError()) {
		window.showErrorMessage(`Geode: Unable to setup Geode extension: ${resBrowser.unwrapErr()}`);
	...
	if (resSave.isError()) {
		window.showErrorMessage(`Geode: Unable to load Geode extension save data: ${resSave.unwrapErr()}`);
	}

    // Webview views
    context.subscriptions.push(
        window.registerWebviewViewProvider("geode-tools.client", new Client(), viewOptions),
	...
	context.subscriptions.push(
        commands.registerCommand("geode.launchGD", async () => {
			...
			const res = await cli.launchProfile(profile.getName());
...
export function deactivate() {
	const saveResponse = saveData();
    
    ViewProvider.INSTANCES.forEach((view) => view.dispose());
    GeodeCLI.get()?.destroyTerminal();

	if (saveResponse.isError()) {
		window.showErrorMessage(`Geode: Unable to save Geode extension data: ${saveResponse.unwrapErr()}`);
	}
}
