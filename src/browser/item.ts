...
import {
	insertSnippet,
	snippet,
	createPlaceholderName as phn,
	createPlaceholder as ph,
} from "../project/source";
...
export async function fetchItemImage<T extends ItemType>(
	item: Item<T>,
): Future<string> {
	switch (item.type) {
		...
		case ItemType.audio: {
			return Ok(
				readFileSync(getAsset("audio-{theme}.png"), {
					encoding: "base64",
				}),
			);
		}
...
export class Collection {
				...
				return (
					(this.sheets as Item<ItemType>[])
						...
						.filter(
							(i) =>
								this.id !== "favorites" ||
								browser.getDatabase().isFavorite(i),
						)
				);
			}
			case "sheets":
				return this.sheets;
			case "frames":
				return this.sheets.flatMap((s) => s.items);
			case "sprites":
				return this.sprites;
			case "fonts":
				return this.fonts;
	...
	findByName(name: string): Option<Item<ItemType>> {
		...
		return (
			this.sprites.find((i) => i.name === name) ??
	...
	add(item: Item<ItemType>) {
		switch (item.type) {
			...
			case ItemType.sheetSprite:
				{
					this.sheets
						.find((s) => s.name === (item as SheetSpriteItem).sheet)
			...
			return [
				{
					...
					snippet: snippet`
                        CCSprite::createWithSpriteFrameName(${getItemUseText(
							item,
						)})
                    `,
				},
				{
					...
					snippet: snippet`
                        auto ${phn(item, 0, "Spr")} = CCSprite::createWithSpriteFrameName(${getItemUseText(
							item,
						)});
				...
				},
				{
					...
					snippet: snippet`
                        auto ${phn(item, 0, "BtnSpr")} = CCSprite::createWithSpriteFrameName(${getItemUseText(
							item,
						)});
		...
		case ItemType.sprite: {
			return [
				...
				},
				{
					name: "Create CCSprite & Add Child",
					snippet: snippet`
                        auto ${phn(item, 0, "Spr")} = CCSprite::create(${getItemUseText(
							item,
						)});
                        ${ph("this", 1)}->addChild(${phn(item, 0, "Spr")});
                    `,
				},
				{
					name: "Create Button & Add Child",
					snippet: snippet`
                        auto ${phn(item, 0, "BtnSpr")} = CCSprite::create(${getItemUseText(
							item,
						)});
				...
				},
				{
					...
					snippet: snippet`
                        auto ${phn(item, 0, "BtnSpr")} = ButtonSprite::create(${ph(
							'"Hi mom!"',
							1,
		...
		case ItemType.font: {
			return [
				...
				},
				{
					...
					snippet: snippet`
                        auto ${phn(item, 0, "Label")} = CCLabelBMFont::create(${ph(
							'"Hi mom!"',
							1,
						)}, ${getItemUseText(item)});
