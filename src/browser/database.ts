...
export class Database {
				...
				readFileSync(sheetPath)
					...
					?.forEach((match) => {
						...
						if (match.replace(".png", ".plist") !== sheetName) {
							...
							if (
								!sheet.items.some((spr) => spr.name === match)
							) {
								sheet.items.push({
			...
			// find sprites
				...
				if (
					collection.sheets.some(
						(sheet) =>
							sheet.name === fileName.replace(".png", ".plist"),
					)
				) {
					continue;
				}
				// is this a font?
				if (
					collection.fonts.some(
						(fnt) => fnt.name === fileName.replace(".png", ".fnt"),
					)
				) {
					continue;
				}
	...
	private addSpritesFromMod(mod: Project) {
		...
		Object.entries(mod.modJson.resources.spritesheets ?? {})?.forEach(
			([sheet, patterns]) => {
				collection.sheets.push({
					...
					src: mod,
					items: patterns
						.flatMap((p) => G.glob.sync(p, globOptions))
						.map((file) => {
							return {
								name: basename(file),
								path: file,
		...
		Object.entries(mod.modJson.resources.fonts ?? {}).forEach(
			([name, font]) => {
				collection.fonts.push({
					name: name,
	...
	removeFavorite(item: Item<ItemType>) {
		this.favorites = this.favorites.filter(
			(f) => !sameLocator(f, getItemLocator(item)),
		);
	}
	...
	getItemByLocator(locator: ItemLocator): Option<Item<ItemType>> {
		return this.collections
			.find((c) => sourceID(c.src) === locator.srcID)
			?.find(locator);
	}
