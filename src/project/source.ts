...
import { Position, Range, SnippetString, TextEditor, window } from "vscode";
import { Item, ItemType } from "../browser/item";
import { Err, Future, Ok, Option } from "../utils/monads";
...
export function createPlaceholderName(item: Item<ItemType>, index: number, ending?: string): Placeholder {
    return new Placeholder(
        camelCase(
            ...
            .replace(/(^[a-zA-Z0-9]{0,3}_)|(_[0-9]+)|((-hd|-uhd)?\.[a-z]+)|(GJ|[0-9]+)/g, '')
        ) + (ending ?? ''),
        index
    );
}
