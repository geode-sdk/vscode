import type { CancellationToken, DocumentColorProvider, ProviderResult, TextDocument } from 'vscode';
import { Color, ColorInformation, ColorPresentation, Position, Range } from 'vscode';
import { getOutputChannel } from '../config';

const MATCH_COLOR_BYTE = /\b((25[0-5])|(2[0-4]\d)|([01]\d\d)|(\d\d?)\b)/g;
...
class CCColorProvider implements DocumentColorProvider {
	...
	private parseColor(color: string): Color | undefined {
		...
		return new Color(
			Number.parseInt(bytes[0]) / 255,
			Number.parseInt(bytes[1]) / 255,
			Number.parseInt(bytes[2]) / 255,
			(this.rgba ? (Number.parseInt(bytes[3]) / 255) : 1),
		);
	}
	...
	provideColorPresentations(color: Color, _context: { readonly document: TextDocument; readonly range: Range }, _token: CancellationToken): ProviderResult<ColorPresentation[]> {
		return [
