...
export function getWebviewUri(webview: Webview, pathList: string[]) {
    return webview.asWebviewUri(Uri.joinPath(getExtContext().extensionUri, ...pathList));
}

export function getWebviewToolkitPath(webview: Webview) {
    return getWebviewUri(
        webview, ['node_modules/@vscode/webview-ui-toolkit/dist/toolkit.min.js']
    );
}
...
export function getCodiconToolkitPath(webview: Webview) {
    return getWebviewUri(
        webview, ['node_modules/@vscode/codicons/dist/codicon.css']
    );
}
